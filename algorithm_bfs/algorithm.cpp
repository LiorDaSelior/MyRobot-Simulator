...
void MyAlgorithm::visitCurrentLocation() {
...
bool MyAlgorithm::checkCurrentLocationDirt() {
    if ((dirt_sensor_ptr -> dirtLevel()) > 0)
        return true;
    house_map.removeCoorDirt(curr_coor);
    return false;
...
Step MyAlgorithm::moveToClosestDirtyLocation() {
...
Step MyAlgorithm::moveToClosestUnvisitedLocation() {
    std::pair<int, int> dst_coor;
...
bool MyAlgorithm::checkIfNeedToReturn(Step planned_step) {
